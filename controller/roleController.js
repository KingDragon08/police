        ...
        User.getUserInfo(mobile, token, function(user){
            if (user.error == 0) {
                ...
                if (check.isNull(roleName)) {
                    res.json({"code": 401, "data":{"status":"fail","error":"roleName is null"}});
                    return ;
                }
                ...
                db.query(sql, dataArr, function(err,rows){
                       if(err){
                           ...
                           if(rows.length > 0 ){
                              res.json({"code": 402, "data":{"status":"fail","error":"role exist"}});
                              return ;
                           }
                           else {
                           ...
                           db.query(sql, dataArr, function(err,rows){
                                  if(err){
                                      ...
                                      res.json({"code": 200, "data":{"status":"success","error":"success", "role_id": rows.insertId}});
                                  }
                              });
                       }
                   });
                ...
                db.query(sql, dataArr, function(err,rows){
                       if(err){
                           ...
                           if(rows[0].total > 0 ){
                               ...
                               res.json({"code": 404, "data":{"status":"fail","error":"role not exist"}});
                           }
                       }
                   });
            }
...
function editRole(req,res){
	...
	try{
        ...
        User.getUserInfo(mobile, token, function(user){
            ...
            var roleName = query.roleName || '';
            if (check.isNull(roleName)) {
                res.json({"code": 401, "data":{"status":"fail","error":"roleName no is null"}});
                return ;
            }
