        ...
        User.getUserInfo(mobile, token, function(user){
            if (user.error == 0) {
                ...
                if (check.isNull(roleId)) {
                    res.json({"code": 401, "data":{"status":"fail","error":"roleId is null"}});
                    return ;
                }

                var actionId = query.actionId || '';
                if (check.isNull(actionId)) {
                    res.json({"code": 401, "data":{"status":"fail","error":"actionId is null"}});
                    return ;
                }
                ...
                db.query(sql, dataArr, function(err,rows){
                       if(err){
                           ...
                           if(rows.length > 0 ){
                              res.json({"code": 402, "data":{"status":"fail","error":"roleaction exist"}});
                              return ;
                           }
                           else {
                           ...
                           db.query(sql, dataArr, function(err,rows){
                                  if(err){
                                      ...
                                      res.json({"code": 200, "data":{"status":"success","error":"success", "roleaction_id": rows.insertId}});
                                  }
                              });
                       }
                   });
        ...
        User.getUserInfo(mobile, token, function(user){
            if (user.error == 0) {
                ...
                if (check.isNull(roleActionId)) {
                    res.json({"code": 401, "data":{"status":"fail","error":"roleActionId is null"}});
                    return ;
                }
                ...
                db.query(sql, dataArr, function(err,rows){
                       if(err){
                           ...
                           if(rows[0].total > 0 ){
                               ...
                               res.json({"code": 404, "data":{"status":"fail","error":"roleaction not exist"}});
                           }
                       }
                   });
            }
                   ...
                   if(err){
                       ...
                       if(rows[0].total > 0 ){
                          ...
                          db.query(sql, dataArr, function(err,rows){
                           ...
                           res.json({"code": 404, "data":{"status":"fail","error":"action not exist"}});
                       }
                   }
               });
        } else {
