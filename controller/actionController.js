        ...
        User.getUserInfo(mobile, token, function(user){
            if (user.error == 0) {
                ...
                if (check.isNull(actionName)) {
                    res.json({"code": 401, "data":{"status":"fail","error":"actionName is null"}});
                    return ;
                }
                       ...
                       if(err){
                           ...
                           db.query(sql, dataArr, function(err,rows){
                                  if(err){
                                      ...
                                      res.json({"code": 200, "data":{"status":"success","error":"success", "action_id": rows.insertId}});
                                  }
                              });
                       }
                   });
...
function editAction(req,res){
	...
	try{
        ...
        User.getUserInfo(mobile, token, function(user){
            ...
            var actionName = query.actionName || '';
            if (check.isNull(actionName)) {
                res.json({"code": 401, "data":{"status":"fail","error":"actionName no is null"}});
                return ;
            }
